...
import com.example.medicalappointment.navigation.screen.BottomNavItemScreen
    ...
    Scaffold(
        ...
        NavHost(
            ...
            composable(route = BottomNavItemScreen.Schedule.route) {
                ScheduleScreen(onError = { errorMessage ->
                    Log.e("ScheduleScreen", "Error: $errorMessage")
                })
            }

            composable(route = BottomNavItemScreen.Profile.route) {
                ProfileScreen(
                    userId = userId, // Inject PatientService
                    onSaveSuccess = {
                        Log.d("ProfileScreen", "Save profile successfully")
                    },
                    onError = { errorMessage ->
                        Log.e("ProfileScreen", "Load profile failed: $errorMessage")
                    }
                )
            }
            composable(route = "doctor_schedule/{doctorId}/{patientId}") { backStackEntry ->
                val doctorId = backStackEntry.arguments?.getString("doctorId")?.toInt() ?: 0
                val patientId = backStackEntry.arguments?.getString("patientId")?.toInt() ?: 0

                DoctorScheduleScreen(
                    doctorId = doctorId,
                    patientId = patientId,  // Truyền thêm patientId
                    onError = { errorMessage ->
                        Log.e("DoctorScheduleScreen", "Error: $errorMessage")
                    },
                    navController = navController
                )
            }
